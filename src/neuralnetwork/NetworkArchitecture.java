...
public class NetworkArchitecture {
     ...
     * illegal argument exception is thrown if the number of nodes in the 
     * last layer does not equal outputDim.
     */
    public NetworkArchitecture(ActivationFunction af, int dataDim, int... numNodesPerLayer) {
        this.actFunc = af;
        ...
        dims[0] = new LayerArchitecture(numNodesPerLayer[0], dataDim, 0);
        for(int i = 1, startInd = dims[0].length(); i < numNodesPerLayer.length; i++, startInd += dims[i - 1].length())
            dims[i] = new LayerArchitecture(numNodesPerLayer[i], dims[i-1].rows, startInd);
    ...
    public Indices getIndex(int ind){
        ...
        if(ind >= matrixStart + dims[matrixInd].numWeights())
            return new Indices(matrixInd, ind - matrixStart - dims[matrixInd].numWeights(), ind);
        else return new Indices(matrixInd, localInd % dims[matrixInd].rows, localInd/dims[matrixInd].rows, ind);
    }
    ...
    public static void main(String[] args) {
        NetworkArchitecture dims = new NetworkArchitecture(new Sigmoid(), 2, 3, 2);
